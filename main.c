...
void getCode(char buff[CHARMAX], data x) {
    buff[x.size] = '\0';
...
void getNewFileName(char nameOut[CHARMAX], char nameIn[CHARMAX], const char * insStr) {
    //char strTime[CHARMAX];
    //time_t curTime = time(NULL);
    //strftime(strTime, CHARMAX, "(%Hh-%Mm-%Ss)", localtime(&curTime));
    int dot = strlen(nameIn) - 1;
    while (nameIn[dot] != '.') dot--;
    strncat(nameOut, nameIn, dot);
    strcat(nameOut, "[");
    strcat(nameOut, insStr);
    strcat(nameOut, "]");
    //strcat(nameOut, strTime);
    if (!strcmp(insStr, "cringed")) {
        strcat(nameOut, ".dnkkn");
    } else {
        strcat(nameOut, ".txt");
    }
...
void decompress() {
    ...
    getNewFileName(nameOut, argv[1], "based");
...
void compress() {
    ...
    char nameOut[CHARMAX] = "\0";
    getNewFileName(nameOut, argv[1], "cringed");
    ...
    printf("[Dankakon:] Successful compression!\nThe new file is called \"%s\"\n", nameOut);
    return;
}
...
void help() {
    printf("[Dankakon:] There are 4 commands now: \"help\", \"compress [path or \\last]\", \"decompress [path or \\last]\", \"exit\"\n");
}

void play() {
    if (!strcmp(argv[1], "rock")) {
        printf("[Dankakon:] Paper! You've lost! HAHHAAHHA!\n");
    } else if (!strcmp(argv[1], "paper")) {
        printf("[Dankakon:] Scissors! You've lost! HAHHAAHHA!\n");
    } else if (!strcmp(argv[1], "scissors")) {
        printf("[Dankakon:] Rock! You've lost! HAHHAAHHA!\n");
    }
}
